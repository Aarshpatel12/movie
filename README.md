...
🎬A Streamlit-based web application that recommends movies based on user preference. The system uses machine learning to calculate similarity between movies and fetches real-time posters using the TMDB API.Getty Images Explore 
🚀 FeaturesSearch & Select: Choose from thousands of movies in the database.Similarity Engine: Uses Cosine Similarity to find the most relevant matches.Dynamic Posters: Fetches high-quality movie posters via The Movie Database (TMDB) API.Responsive UI: A clean, 5-column layout to display recommendations.🛠️ Tech StackFrontend: StreamlitLanguage: Python 3.xLibraries: Pandas, Scikit-learn, Pickle, RequestsAPI: TMDB API
📂Project StructurePlaintext├── model/
